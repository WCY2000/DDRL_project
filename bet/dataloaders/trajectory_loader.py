...
class RelayKitchenMultiviewTrajectoryDataset(Dataset):
    def __init__(self, data_directory, onehot_goals=True):
        ...
        actions = torch.from_numpy(np.load(data_directory / "actions_seq.npy")).float()
        masks = torch.from_numpy(np.load(data_directory / "existence_mask.npy")).bool()
        # The current values are in shape T x N x Dim, move to N x T x Dim
        states, actions, masks = transpose_batch_timestep(states, actions, masks)
        self.states = states
