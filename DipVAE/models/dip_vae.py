...
class DIPVAE(BaseVAE):
    ...
    def forward(self, input: Tensor, **kwargs) -> List[Tensor]:
        ...
        if not TRAIN:
            yaml_file_path = '/home/chenyu/Desktop/visuotactile_representations/DipVAE/configs/dip_vae.yaml'
            with open(yaml_file_path, 'r') as file:
                config = yaml.safe_load(file)
            data_path = config['data_params']['data_path']
            data_path = os.path.join(data_path, 'mu-latent.txt')
            with open(data_path, 'a') as f:
                for row in mu:
                    # Convert the tensor row to a list of numbers and join them into a string
