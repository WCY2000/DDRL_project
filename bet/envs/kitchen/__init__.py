...
class KitchenBase(KitchenTaskRelaxV1):
    ...
    def _get_obs(self):
        state = super(KitchenBase, self)._get_obs()
...
class KitchenMultiviewBase(KitchenBase):
    """Base class for kitchen environments with multiple views."""
    ...
    def __init__(
        ...
        super(KitchenMultiviewBase, self).__init__(**kwargs)

    def _get_obs(self):
        state = super(KitchenMultiviewBase, self)._get_obs()
        imgs = []
        for view in self.VIEWS:
            self.sim_robot.renderer._camera_settings = view
            img = self.sim_robot.renderer.render_offscreen(224, 224, 0, -1)  # HWC
            img = einops.rearrange(img, "H W C -> C H W")
            imgs.append(img)
        # not using proprioception so that the dynamics model must learn from vision
        proprio = state[:7]
